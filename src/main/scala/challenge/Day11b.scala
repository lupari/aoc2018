...
object Day11b extends Challenge {
  ...
  def summedAreaTable(mat: Array[Array[Int]], n: Int): Array[Array[Int]] = {
    val sat  = Array.ofDim[Int](n, n)
    for { i <- 0 until n } yield sat(0)(i) = mat(0)(i)
    for { i <- 1 until n; j <- 0 until n } yield sat(i)(j) = mat(i)(j) + sat(i - 1)(j)
  ...
  override def run(): Any = {
    ...
    lazy val results = for {s <- 1 until n; i <- 0 until n - s; j <- 0 until n - s} yield
