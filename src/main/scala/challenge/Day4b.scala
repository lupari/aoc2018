...
object Day4b extends Challenge {
  ...
  override def run(): Any = {
    ...
    val minuteMapped: Map[String, Map[Int, Int]] =
      shifts.map(s => (s._1, s._2.groupBy(identity).mapValues(_.length)))
    val guard = minuteMapped.maxBy(_._2.values.max)
    val minute: Int = guard._2.maxBy(_._2)._1
