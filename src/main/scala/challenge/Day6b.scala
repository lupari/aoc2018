...
object Day6b extends Challenge {
  ...
  def distanceToLocations(point: Point, locations: List[Point]): Int = {
    locations.map(l => distance(l, point)).sum
  }
  ...
  override def run(): Any = {
    ...
    grid.map(c => (c, distanceToLocations(c, fixedCoords))).count(_._2 < 10000)
  }
